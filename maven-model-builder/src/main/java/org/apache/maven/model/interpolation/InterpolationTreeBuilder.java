...
public class InterpolationTreeBuilder
{
        ...
        throws IllegalAccessException
    {
        ...
        if ( c != null )
        {
            ...
            for ( int i = 0; i < size; i++ )
            {
                ...
                if ( itemValue != null )
                {
                    Class<? extends Object> classOfValye = itemValue.getClass();
    ...
    private void buildArrayItem( List<Element> children, Field currentField, int i, Object item )
        throws IllegalAccessException
    {
        if ( item instanceof String )
        {
    ...
    private static boolean isQualifiedForInterpolation( Class<?> cls )
    {
        return !cls.getName().startsWith( "java" );
    }

    private boolean isFieldQualifiedForInterpolation( Field field )
    {
        return !field.getType().isPrimitive() && !"parent".equals( field.getName() );
